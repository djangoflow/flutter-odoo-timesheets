...
class LoginListener extends BlocListener<AuthCubit, AuthState> {
  LoginListener({
    Key? key,
    void Function(BuildContext context, OdooUser user)? onLogin,
    void Function(BuildContext context)? onLogout,
    Widget? child,
  }) : super(
            key: key,
            listener: (context, authState) => authState.odooUser != null
                ? onLogin?.call(context, authState.odooUser!)
            ...
            listenWhen: (prev, next) =>
                (next.odooUser?.id != prev.odooUser?.id));
}
